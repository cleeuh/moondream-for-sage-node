...
@dataclass
class OnnxVL_DYNAMIC(VLM):
    # A model for resource limited sessions, loads and unloads on demand
    model_path: str
    tokenizer: Tokenizer
    ...
    def model_run(self, model_name, output_names, input_feed, run_options=None):
        model = self.model_load(model_name)
        results = model.run(output_names, input_feed, run_options=None)
        del model # Make extra sure the model is unloaded
        return results

    def model_load(self, model_name):
        ort.set_default_logger_severity(3)

        if ort.get_device() == "GPUA": # NO GPU for now
            print("USING GPU")
        ...
        for filename, contents in unpack(self.model_path):
            key, ext = filename.split("/")[-1].split(".")
            if key == model_name:
                return ort.InferenceSession(contents, **ort_settings)

        raise ValueError("Encoder/Decoder not found in model file") 



    def run_decoder(self, input_embeds, kv_cache, pos):
        """
        ...
        hidden, kv_cache_update, logits = self.model_run(
            "text_decoder",
    ...
    @classmethod
    def from_path(cls, model_path: str) -> "OnnxVL_DYNAMIC":
        ...
        print(components["config"]["templates"])
        return cls(
    ...
    def encode_image(self, image: Union[Image.Image, EncodedImage]) -> EncodedImage:
        ...
        (patch_emb,) = self.model_run("vision_encoder", None, {"input": image_patches})
    ...
    def _generate(
        ...
        text_decoder = self.model_load("text_decoder")
        text_encoder = self.model_load("text_encoder")

        while generated_tokens < max_tokens:
            logits, kv_cache_update = text_decoder.run(
                ["logits", "new_kv_cache"],
                {
                    "input_embeds": input_embeds,
                    "kv_cache": kv_cache[:, :, :, :, :pos, :],
            ...
            generated_tokens += 1
            (input_embeds,) = text_encoder.run(None, {"input_ids": [[next_token]]})
            input_length = 1
    ...
    def caption(
        ...
        (input_embeds,) = self.model_run(
            "text_encoder", None, {"input_ids": [self.templates["caption"][length]]}
        )
        if settings is None:
            settings = {}
    ...
    def query(
        ...
        (input_embeds,) = self.model_run("text_encoder", None, {"input_ids": [prompt_toks]})
        if settings is None:
            settings = {}
        max_tokens = settings.get("max_tokens", DEFAULT_MAX_TOKENS)
    ...
    def detect(
        ...
        (hidden,) = self.model_run("text_encoder", None, {"input_ids": [prompt_toks]})
        encoded_image = self.encode_image(image)
        assert type(encoded_image) == OnnxEncodedImage
        kv_cache = prepare_kv_cache(encoded_image)
        ...
        while len(objects) < max_objects:
            logits, hidden = self.run_decoder(hidden, kv_cache, pos)
            pos += hidden.shape[-2]
            ...
            (x_center,) = self.model_run("coord_decoder", None, {"input": hidden[0, -1, :]})
            x_center = np.argmax(x_center, axis=-1) / x_center.shape[-1]
            (hidden,) = self.model_run("coord_encoder", None, {"input": [x_center]})
            hidden = np.expand_dims(np.expand_dims(hidden, 0), 0)

            # Decode and encode y center coordinate.
            logits, hidden = self.run_decoder(hidden, kv_cache, pos)
            pos += hidden.shape[-2]
            (y_center,) = self.model_run("coord_decoder", None, {"input": hidden[0, -1, :]})
            y_center = np.argmax(y_center, axis=-1) / y_center.shape[-1]
            (hidden,) = self.model_run("coord_encoder", None, {"input": [y_center]})
            hidden = np.expand_dims(np.expand_dims(hidden, 0), 0)

            # Decode and encode size.
            logits, hidden = self.run_decoder(hidden, kv_cache, pos)
            pos += hidden.shape[-2]
            (size,) = self.model_run("size_decoder", None, {"input": hidden[0, -1, :]})
            w = np.argmax(size[0], axis=-1) / size.shape[-1]
            h = np.argmax(size[1], axis=-1) / size.shape[-1]
            (hidden,) = self.model_run("size_encoder", None, {"input": [w, h]})
            hidden = np.expand_dims(np.expand_dims(hidden, 0), 0)
    ...
    def point(
        ...
        while len(points) < max_points:
            ...
            # Decode and encode x center coordinate.
            (x,) = self.model_run("coord_decoder", None, {"input": hidden[0, -1, :]})
            x = np.argmax(x, axis=-1) / x.shape[-1]
            (hidden,) = self.model_run("coord_encoder", None, {"input": [x]})
            hidden = np.expand_dims(np.expand_dims(hidden, 0), 0)

            # Decode and encode y center coordinate.
            logits, hidden = self.run_decoder(hidden, kv_cache, pos)
            pos += hidden.shape[-2]
            (y,) = self.model_run("coord_decoder", None, {"input": hidden[0, -1, :]})
            y = np.argmax(y, axis=-1) / y.shape[-1]
            (hidden,) = self.model_run("coord_encoder", None, {"input": [y]})
            hidden = np.expand_dims(np.expand_dims(hidden, 0), 0)
